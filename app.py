import dash
import numpy as np
from dash import dccimport dash
import numpy as np
...
searchbar=dbc.Label("SkillAssist"),dbc.Input(id="input",debounce=True, placeholder="Type the skills you wanna learn", type="text",className="bi bi-search"),dbc.FormText("SkillAssist helps you with finding online courses",color="LimeGreen",style={'font-weight':'600'})
...
def render_page_content(pathname):
    if pathname=="/":
        ...
        return [dbc.Container([dbc.Col([html.Div(searchbar),dbc.Row([dbc.Col([html.H2("Youtube Courses",style={'color':'Crimson'}),html.Div(id="output")]),dbc.Col([html.Div([html.H5("Average Views for this course",style={'font-weight':'bold'},className="p-2"),html.H5('%.2f'%views,style={'font-weight':'bold','color':'Crimson'},className="p-2 bi bi-bar-chart-line-fill")],className="card"),html.H5("View Distribution",style={'font-weight':'bold','color':'Crimson','margin-top':'30px'}),dcc.Graph(figure=px.treemap(df,path=["videotitle","views"],values="views",color="videotitle",template="plotly_dark"))])])])])]
